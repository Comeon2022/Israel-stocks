...
const buildCompanySpecificFairValueFixed = (companyId: string, rows: any[], market: any, retailer: boolean, fv1: any) => {
  ...
  const normalizedEbit = fv1.normalization?.ebit?.value as number | null, normalizedNetIncome = fv1.normalization?.netIncome?.value as number | null, shares = fv1.basis?.sharesOutstanding as number | null, currentPrice = n(market?.share_price)
