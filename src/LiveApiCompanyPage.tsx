...
function Metric({ label, metric, showHelp = false, value, reason }: { label: string; metric?: string; showHelp?: boolean; value: React.ReactNode; reason?: { primary: string; secondary?: string } }) {
...
function PeerEducation({ metric, shown }: { metric: string; shown: string }) {
  if (metric === 'cashConversion') return <>
    ...
    <p dir="ltr">Cash Conversion = Free Cash Flow / Net Income — כמה מהרווח הנקי הופך לתזרים מזומנים חופשי</p>
    <h4>מה זה?</h4>
    <p>המרת רווח למזומן מודדת עד כמה הרווח הנקי שהחברה מדווחת עליו מתורגם בפועל לתזרים מזומנים חופשי.</p>
    <p>תזרים מזומנים חופשי (FCF) הוא המזומן שנשאר לחברה לאחר הפעילות השוטפת ולאחר ההשקעות ההוניות הנדרשות לעסק.</p>
    <p>במילים פשוטות, המדד עוזר לבדוק אם הרווח שמופיע בדוחות מגובה גם בתזרים מזומנים חופשי שנשאר לחברה.</p>
    <h4>איך קוראים את המספר?</h4>
    <p>אם יחס המרת הרווח למזומן הוא {shown}%, זה אומר שהחברה מייצרת תזרים מזומנים חופשי בגובה של בערך {shown}% מהרווח הנקי שלה.</p>
    <p>במילים פשוטות: על כל 1 ₪ של רווח נקי, החברה מייצרת בערך {(Number(shown) / 100).toFixed(2)} ₪ של תזרים מזומנים חופשי.</p>
    <h4>איך מפרשים את זה?</h4>
    <p>יחס קרוב ל־100% יכול להעיד שהרווח הנקי מתורגם בצורה טובה יחסית לתזרים מזומנים חופשי.</p>
    <p>יחס גבוה מ־100% יכול לקרות כאשר החברה מייצרת יותר FCF מהרווח הנקי המדווח, למשל בגלל הוצאות לא־מזומניות כמו פחת, שיפור בהון החוזר או השקעות הוניות נמוכות יחסית בתקופה.</p>
    <p>יחס נמוך משמעותית מ־100% לאורך זמן יכול להעיד שחלק גדול מהרווח אינו מתורגם לתזרים מזומנים חופשי.</p>
    <p>עם זאת, יחס נמוך בתקופה מסוימת אינו בהכרח בעיה. הוא יכול לנבוע מגידול במלאי, השקעה בהון חוזר, עונתיות או אירוע חד־פעמי.</p>
    <p>לכן חשוב לבדוק את המדד לאורך כמה שנים יחד עם הרווח הנקי, CFO, Capex ו־FCF.</p>
  </>
  ...
  if (metric === 'fcfMargin') return <>
    ...
    <p>במילים פשוטות: מכל 1 ₪ של הכנסות, נשארים לחברה בערך {Math.round(Number(shown))} אגורות כתזרים מזומנים חופשי.</p>
    <h4>איך מפרשים את זה?</h4>
  ...
  if (metric === 'netMargin') return <>
    ...
    <p>במילים פשוטות: מכל 1 ₪ של הכנסות, נשארים לחברה בערך {Math.round(Number(shown))} אגורות כרווח נקי.</p>
    <h4>איך מפרשים את זה?</h4>
  ...
  if (metric === 'ebitMargin') return <>
    ...
    <p>במילים פשוטות: מכל 1 ₪ של הכנסות, נשארים לחברה בערך {Math.round(Number(shown))} אגורות כרווח תפעולי.</p>
    <h4>איך מפרשים את זה?</h4>
  ...
  if (metric === 'fcfYield') return <>
    ...
    <p>במילים פשוטות: על כל 100 ₪ של שווי שוק, החברה מייצרת כיום תזרים מזומנים חופשי שנתי של כ־{shown} ₪.</p>
    <h4>איך מפרשים את זה?</h4>
  ...
  if (metric === 'evEbitda') return <>
    ...
    <p>EV — Enterprise Value הוא שווי הפעילות של העסק. בפשטות ובקירוב, הוא מתחיל משווי השוק של החברה, מוסיף חוב פיננסי ומפחית מזומן.</p>
