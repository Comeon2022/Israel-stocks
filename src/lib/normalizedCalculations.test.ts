...
import { canonicalCompanyId, companyRoute, findCompany } from '../data/companies'
...
describe('normalized financial math', () => {
  ...
  it('normalizes Sano identity for routes and lookups', () => { expect(canonicalCompanyId(' SANO ')).toBe('sano'); expect(companyRoute({ id: 'sano' })).toBe('/company/sano'); expect(findCompany('SANO')?.id).toBe('sano'); expect(findCompany('sano')?.id).toBe('sano') })
})
