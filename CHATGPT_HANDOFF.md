...
Worker deployment: `9b4c3279-eb0c-4ab2-83a4-6c7a66f40a06` at `https://israel-stocks-api.karu-lior.workers.dev`. Pages deployment: `https://07525d41.israel-stocks.pages.dev`, source commit `d0ca5ae`. Chrome CDP verified all five company routes as React-rendered, market-backed, mock-free, TTM-unavailable, and without console/runtime errors. `npm test`, `npm run worker:test`, `npm run worker:check`, and `npm run build` passed. Unrelated `.gitignore` and `buildorder/` remain untouched.
