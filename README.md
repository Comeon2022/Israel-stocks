...
MAYA requests are shape-checked and retried up to three times with bounded backoff. Current live API rejects the CLI's historical page size of 100 with HTTP 400 (`PageSize` must be 1–30); peer activation remains blocked until the CLI request is reduced to the server limit and wired to the shared D1 persistence service.
